window.HUDScene = class {
    ...
    constructor(satelliteTracker) {
        ...
        this.timeline.push({"speed": 1, "name": "REAL-TIME SPEED"});
    ...
    updateHUD(date, mouseX, mouseY) {
        ...
        if (this.satelliteTracker.loadingProgress.isReady()) {
            ...
            if (focusedSatellite !== undefined) {
                ...
                if (!this.satelliteTracker.renderer.isMobile) {
                    ...
                    if (!this.satelliteTracker.focusedEarth && !this.hoverToggleEarthFocusButton) {
                        this.drawHint(width - 40, height - 40, "DEVELOPED BY TURING TECH", '#666666', '#ffffff', 0.4);
                    }
                }
            }
            ...
            this.drawRect(0, 0, width, height, 'rgba(0, 0, 0, 0)');
...
// ############ Right bottom buttons ############
    ...
    drawEarthFocusButton(x, y, size, mouseX, mouseY) {
        ...
        if (mouseOver) {
            this.drawRightText(x - 5, y + size / 2 + 4, "Focus " + (this.satelliteTracker.focusedEarth ? "ISS" : "Planet Earth"), '#ffffff', 20, false);
        }
    }
    ...
    drawDockingButton(x, y, size, mouseX, mouseY) {
        ...
        if (mouseOver) {
            this.drawRightText(x - 5, y + size / 2 + 4, (this.showDockingInformation ? "Hide" : "Show") + " docked spacecrafts", '#ffffff', 20, false);
        }
    }
    ...
    drawTimeButton(x, y, size, mouseX, mouseY) {
        ...
        if (mouseOver) {
            this.drawRightText(x - 5, y + size / 2 + 4, (this.showTimeline ? "Hide" : "Show") + " orbit timeline speed", '#ffffff', 20, false);
        }
    }
...
// ############ Docking list ############
    ...
    drawDockingList(docking, x, y) {
        ...
        for (const index in list) {
            ...
            this.drawRightText(x - maxValueLength - 5, y, satellite.port.name + ":", '#000080', fontSize, false);
...
// ############ Progressbar ############
    ...
    drawProgressbar(x, y, width, height, percentage) {
        ...
        this.drawRect(x, y, x + progress, y + height, '#000080')
    }
...
// ############ Timeline ############
    ...
    drawTimeline(x, y, width, height, mouseX, mouseY, date) {
        ...
        let stateString = currentSpeed === 1 ? "REAL-TIME SPEED" : (currentSpeed + "x SPEED");
        ...
        // Live state
        let color = isLive ? '#000080' : this.hoverLive ? '#ffffff' : '#888888';
        this.drawRoundRect(x + width / 2 + 100, y + height + 5, 6 * 2, 6 * 2, 6, 0, color, color);
        this.drawText(x + width / 2 + 115, y + height + 17, "REAL-TIME", color, 20, isLive);
    }
...
// ############ Telemetry ############
    ...
    drawTelemetry(satellite, x, y, width, height, date) {
        ...
        if (state.hasCrashed()) {
            ...
            this.drawSpeedometer(x + 100, y + 90, state.getSpeed(), 30000, "CURRENT SPEED", "KM/H");
            this.drawSpeedometer(x + 240, y + 90, state.altitude, 460, "CURRENT ALTITUDE", "KM");

            this.drawCenteredText(x + 100 + (280 - 100) / 2, y + height - 8, satellite.name + "ACTIVE TELEMETRY DATA FROM NORAD", '#ffffff', 14, false);
        }
    }
  ...
  drawSpeedometer(x, y, value, maxValue, title, unit) {
        ...
        // Track line
        this.drawArc(x, y, radius, arcStart + arcWhiteProgress, arcEnd - arcRedRange, arcThickness, 'rgba(0, 0, 0, 0)');
        this.drawArc(x, y, radius, arcEnd - arcRedRange + arcRedProgress, arcEnd, arcThickness, 'rgba(0, 0, 0, 0)');

        // Progress
        this.drawArc(x, y, radius, arcStart, arcStart + arcWhiteProgress, arcThickness, 'rgba(0, 0, 0, 0)');
        this.drawArc(x, y, radius, arcEnd - arcRedRange, arcEnd - arcRedRange + arcRedProgress, arcThickness, 'rgba(0, 0, 0, 0)');

        // Draw start hook
        this.drawArc(x, y, radius - 3, arcStart - 1, arcStart + 1, 12, 'rgba(0, 0, 0, 0)');
        ...
        // Labels
        this.drawCenteredText(x, y - 35, title, '#ffffff', 20, true);
        this.drawCenteredText(x, y + 8, labelValue, '#ffffff', 45, false);
        this.drawCenteredText(x, y + 30, unit, '#ffffff', 20, true);
    }
